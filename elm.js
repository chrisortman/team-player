...
Elm.Player.make = function (_elm) {
   ...
   var playing = F2(function (model,
   m) {
      return A2($List.member,
      m,
      model.playingMinutes);
   });
   var subIn = F2(function (model,
   m) {
      return function () {
         switch (m)
         {case 20: return A2(playing,
              model,
              m);}
         return $Basics.not(A2(playing,
         model,
         m + 1)) ? A2(playing,
         model,
         m) : false;
   ...
   model) {
      return function () {
         ...
         {case "None": return model;
            case "SubIn":
            return _U.replace([["playingMinutes"
                               ,A2($Debug.log,
                               "gameMinutes",
                               A2($List._op["::"],
                               action._0,
   ...
   var rowView = F3(function (address,
   gameMinutes,
   model) {
      return function () {
         var totalMinutes = $Array.length($Array.fromList(model.playingMinutes));
         var isPlaying = function (minute) {
            return A2($List.member,
            minute,
            model.playingMinutes);
         };
         var cell = function (minute) {
            return A2($Html.td,
            _L.fromArray([cellStyle(isPlaying(minute))
                         ,A2($Html$Events.onClick,
                         address,
                         isPlaying(minute) ? SubOut(minute) : SubIn(minute))]),
         ...
         _L.fromArray([$Html.text(model.name)]))]),
         A2($Basics._op["++"],
         A2($List.map,cell,gameMinutes),
         _L.fromArray([A2($Html.td,
         _L.fromArray([]),
         _L.fromArray([$Html.text($Basics.toString(totalMinutes))]))]))));
      }();
   });
   var None = {ctor: "None"};
...
Elm.PlayerList.make = function (_elm) {
   "use strict";
   _elm.PlayerList = _elm.PlayerList || {};
   if (_elm.PlayerList.values)
   ...
   $Signal = Elm.Signal.make(_elm),
   $String = Elm.String.make(_elm);
   var reverseMap = F2(function (f,
   l) {
      return function ($) {
         return $List.reverse($List.map(f)($));
      }(l);
   });
   var viewSublist = F2(function (address,
   model) {
      return function () {
         var nonBlankString = function (s) {
            return $Basics.not($String.isEmpty(s));
         };
         var formatSubs = function (pairs) {
         ...
         var subsIn = function (m) {
            return $List.map(function (_v16) {
               return function () {
                  switch (_v16.ctor)
                  {case "_Tuple2":
         ...
         var subsForMinute = function (m) {
            return function () {
               var subs = subPairs(m);
               var subMessage = $String.concat(_L.fromArray([$Basics.toString(m)
                                                            ,":00 - "
                                                            ,formatSubs(subs)]));
               return $Basics.not($List.isEmpty(subs)) ? subMessage : "";
            }();
         };
         var substitutions = $List.filter(nonBlankString)(A2(reverseMap,
         subsForMinute,
         model.minutes));
         return A2($Html.ul,
         _L.fromArray([]),
   ...
   var playingCount = F2(function (model,
   minute) {
      return function () {
         var isPlaying = function (p) {
            return A2($Player.playing,
            p,
            minute);
         };
         return $Array.length($Array.fromList($List.filter(isPlaying)(A2($List.map,
         function (_v20) {
            return function () {
               switch (_v20.ctor)
               {case "_Tuple2":
   ...
   model) {
      return function () {
         ...
         {case "Add":
            return function () {
                 var genericName = A2($Basics._op["++"],
                 "Player ",
                 $Basics.toString(model.nextID));
                 var newPlayer = {ctor: "_Tuple2"
                                 ,_0: model.nextID
                                 ,_1: $Player.init(genericName)};
   ...
   model) {
      return function () {
         var playerRow = function (_v36) {
            return function () {
               switch (_v36.ctor)
               {case "_Tuple2":
                  return A3($Player.rowView,
                    A2($Signal.forwardTo,
                    address,
                    Sub(_v36._0)),
         ...
         playerRow,
         model.players);
         var prependEmptyCell = function (rows) {
            return A2($List._op["::"],
            A2($Html.td,
            _L.fromArray([]),
            _L.fromArray([])),
         ...
         var headerCell = function (value) {
            return A2($Html.th,
            _L.fromArray([]),
            _L.fromArray([$Html.text($Basics.toString(value))]));
         };
         var headerRow = _L.fromArray([A2($Html.tr,
         _L.fromArray([]),
         A2($Basics._op["++"],
         _L.fromArray([headerCell("Player")]),
         A2($Basics._op["++"],
         A2(reverseMap,
         headerCell,
         model.minutes),
         _L.fromArray([headerCell("Minutes")]))))]);
         var valueCell = function (value) {
            return A2($Html.td,
            _L.fromArray([]),
            _L.fromArray([$Html.text($Basics.toString(value))]));
         };
         var footerRow = _L.fromArray([A2($Html.tr,
         _L.fromArray([]),
         prependEmptyCell($List.map(valueCell)($List.map(playingCount(model))($List.reverse(model.minutes)))))]);
